...
{
  static BIO_METHOD *proxy_biom = NULL;

  if (proxy_biom == NULL)
    {
    proxy_biom = BIO_meth_new(BIO_get_new_index(), "proxy");
    BIO_meth_set_write(proxy_biom, proxy_write);
    BIO_meth_set_read(proxy_biom, proxy_read);
    BIO_meth_set_puts(proxy_biom, proxy_puts);
    BIO_meth_set_gets(proxy_biom, proxy_gets);
    BIO_meth_set_ctrl(proxy_biom, proxy_ctrl);
    BIO_meth_set_create(proxy_biom, proxy_new);
    BIO_meth_set_destroy(proxy_biom, proxy_free);
    BIO_meth_set_callback_ctrl(proxy_biom, proxy_callback_ctrl);
    }
  return proxy_biom;
}
